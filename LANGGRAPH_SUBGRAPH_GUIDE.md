...
1. **Keep Subgraphs Simple**: Focus on summarization, let chat_node handle guidance
2. **Use State Dict**: Add fields dynamically, don't modify AgentState class
3. **Emit Messages**: Always use `copilotkit_emit_message` for user-facing messages
4. **Tool Integration**: Call tools directly for simple cases, use ToolNode for complex flows
5. **Error Handling**: Wrap tool calls in try/except with fallbacks
6. **State Persistence**: State fields persist automatically with checkpointing
