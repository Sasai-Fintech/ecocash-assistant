...
// Get API base URL - handle both production and development
const getApiBase = () => {
  if (typeof window !== 'undefined') {
    // Client-side: use the same origin or configured URL
    const remoteUrl = process.env.NEXT_PUBLIC_REMOTE_ACTION_URL;
    if (remoteUrl) {
      return remoteUrl.replace("/api/copilotkit", "");
    }
 ...
 * Sessions are now queried from the backend (PostgreSQL) instead of localStorage
 */
export function useSessionTitle(threadId: string | null) {
  ...
  useEffect(() => {
    const loadSessions = async () => {
      try {
        setLoading(true);
        const apiUrl = `${API_BASE}/api/sessions/`;
        console.log("[useSessionTitle] Fetching sessions from:", apiUrl);
        console.log("[useSessionTitle] API_BASE:", API_BASE);
        ...
        if (response.ok) {
          ...
          const converted: SessionInfo[] = data.map((s: any) => {
            if (!s.thread_id) {
  ...
  const deleteSession = async (sessionThreadId: string) => {
    try {
      // Delete from backend
      const response = await fetch(`${API_BASE}/api/sessions/${sessionThreadId}`, {
        method: "DELETE",
      });
